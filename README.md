...
| Tests | 107 passing, and 131 on Linux where the Wayland tests run |
...
It opens, paints, follows a resize and exits when closed. What it paints is the
specimen: both faces, the palette, and every primitive the wall will be built
out of. `--frame PATH` renders the same frame to a PPM and exits, which needs
no compositor at all — the way to look at the drawing on a headless node, in a
container, or on a Mac. `--dark` picks the night palette.

**The drawing is all software and there is no renderer underneath it.** A
shared-memory surface is a flat array of pixels, so `draw.rs` writes them. On a
Zero 2 that is the right trade twice: a wall of flat rectangles and small text
costs almost nothing to rasterise, and the EGL stack it avoids is both a
dependency and a thing that fails differently on every board.

**The letters are baked in.** A node has no font package, no fontconfig and no
internet, so `src/font.rs` carries two faces of the X.Org *misc-fixed* font —
8x13 and 10x20, whose own COPYRIGHT property reads *"Public domain font. Share
and enjoy."* It is generated by `tools/mkfont.py` from the PCF and committed,
because `copal-build` on a node must not need Python or a font package to
compile a console. Past Latin-1 exactly five characters are baked, each of
which the console actually prints; the list and the reasons are at the top of
`tools/mkfont.py`.

**The palette is the page's.** `assets/wall.html` already carries a designed
theme and `docs/THEME.md` is where it came from, so the native console uses
those values rather than inventing a second set.
...
| `src/draw.rs` | the software canvas: the only drawing this program does |
| `src/font.rs` | the glyphs, baked in — generated, not written |
